...
Page({
  ...
  setDropItem(event) {
    const imgData = event.currentTarget.dataset.item
    let data = {}
    wx.getImageInfo({
      src: imgData.url,
      success: (res) => {
        ...
        data.id = ++itemId //id
        data.top = res.width / 2 //top定位
        data.left = res.height / 2 //left定位
        //圆心坐标
  ...
  oTouchStart(e) {
    ...
    //获取图片半径
    items[index].r = this.getDistancs(
      items[index].x,
      items[index].y,
      items[index].left,
      items[index].top
  ...
  synthesis() {
    ...
    items.forEach((currentValue, index) => {
      ...
      maskCanvas.translate(
        -((currentValue.width * currentValue.scale * prop) / 2),
        -((currentValue.height * currentValue.scale * prop) / 2)
      )
      maskCanvas.drawImage(
        currentValue.image,
        0,
        0,
        currentValue.width * currentValue.scale * prop,
        currentValue.height * currentValue.scale * prop
      )
      maskCanvas.restore()
    })
  ...
  // 取消选中 把最后一张图在canvas置顶显示
  cancelActive(event, jump) {
    if (event?.target?.dataset?.type === 'empty' || jump) {
      const lastItemIndex = items.findIndex((curr) => curr.active)
      let itemsTemp = JSON.parse(JSON.stringify(items))
      if (lastItemIndex !== -1) {
        const lastItem = JSON.parse(JSON.stringify(items[lastItemIndex]))
        itemsTemp = items.filter((curr) => !curr.active)
        itemsTemp.push(lastItem)
      }
      for (let i = 0; i < items.length; i++) {
        itemsTemp[i].active = false
      }
      items = itemsTemp
      this.setData({
        itemList: items,
